...
export class RecognitionService implements OnModuleInit {
  ...
  private async attachTexts(
    ...
    // Зона близости по правилу Максима: рамка элемента, расширенная на две
    // её ширины и высоты в каждую сторону. Отступ считается от размера самого
    // аппарата, а не фиксированными пикселями: у мелкого элемента подпись
    // рядом, у крупного — дальше, и общая мерка промахивалась в обе стороны.
    // Кусок берём по пересечению, а не по полному вхождению: строка, начатая
    // в зоне, учитывается целиком — это его же оговорка про продолжение.
    let attached = 0;
    for (const el of els) {
      const b = el.bbox;
