...
function SlideMedia({ slide }: { slide: OnboardingSlide }) {
  ...
  return (
    <div style={{ ...wrapStyle, background: '#f4f4f4' }}>
      <img
        src={slide.mediaUrl}
...
/**
 * Step-by-step onboarding shown once to new users ~2s after the spec page
 * opens. Each slide carries an optional image/video plus a title and text.
    ...
    >
      ...
      >
        ...
        <div style={{ padding: 20 }}>
          ...
          {slide.title && (
            <h2 style={{ fontSize: 20, fontWeight: 800, margin: '18px 0 8px' }}>{slide.title}</h2>
          )}
          {slide.description && (
            <p style={{ fontSize: 14, color: '#4b5563', lineHeight: 1.6, margin: 0, whiteSpace: 'pre-wrap' }}>
              {slide.description}
            </p>
          )}
        </div>
        ...
        <div style={{
          ...
          gap: 12, padding: '14px 20px', borderTop: '1px solid #eee',
        }}>
          {/* Dots */}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {!isFirst && (
              <button
                ...
                style={{
                  padding: '9px 16px', background: '#f3f4f6', color: '#1a1a1a',
